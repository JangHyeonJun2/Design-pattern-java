...
[옵저버 패턴 예제 바로가기](https://github.com/JangHyeonJun2/Design-pattern-java/tree/master/src/main/java/case5)
...
- 일반 상품을 예쁜 포장지로 싸서 선물용 상품으로 만드는 것처럼, 무엇인가를 한번 초장해서 다른 용도로 사용할 수 있게 교환해 주는 것이 wrapper이며 adapter 이다.

#### Adapter 패턴에는 다음과 같이 두 가지 종류가 있다.

- 클래스에 대한 Adapter 패턴 ( 상속을 사용한 Adapter 패턴)
...
- 메소드가 필요하면 그것을 프로그래밍하면 되지 Why??  Adapter 패턴이 필요한 것일까?

우리는 언제나 처음부터 프로그래밍을 한다고 할 수는 없다. 이미 존재하고 있는 클래스를 이용하는 경우도 자주 있다. 특히 그 클래스가 충분한 테스트를 받아서 버그가 적으면 실제로 지금까지 사용된 실적이 있다면 어떻게든 그 클래스를 부품으로 재이용하고 싶을 것이다.

#### Adapter 패턴은 기존의 클래스를 개조해서 필요한 클래스를 만든다.

- 이 패턴으로 필요한 메소드를 발빠르게 만들 수 있다.
- 만약 버그가 발생해도 기존의 클래스에는 버그가 없으므로 Adapter 역할의 클래스를 중점적으로 조사하면 되고, 프로그래밍 검사도 상당히 쉬워진다.

[어댑터 패턴 예제 바로가기](https://github.com/JangHyeonJun2/Design-pattern-java/tree/master/src/main/java/adapter)
